...
//Styles
import styles from '../styles/TaxiRentWithPurchase.module.scss'
...
import {fetchPage, usdCourse} from "../redux/actions";
...
export default function Home() {
  ...
  //Calculator
  const [autoPrice, setAutoPrice] = useState([15000])
  const [firstPayment, setFirstPayment] = useState([3000])
  const [term, setTerm] = useState(104)

  //Api
  const dispatch = useDispatch()
  const usd = useSelector((state) => state.usdCourse)
                ...
                <div className={styles.calc} uk-scrollspy="cls: uk-animation-fade">
                  <div className={styles.calc__left}>
                    <div className={styles.calc__left_top}>
                      ...
                      <Range
                        ...
                        max={50000}
                        values={autoPrice}
                        onChange={(value) => setAutoPrice(value)}
                      ...
                      <Range
                        ...
                        max={7000}
                        values={firstPayment}
                        onChange={(value) => setFirstPayment(value)}
                        renderTrack={({ props, children }) => (
                          <div
                            className={styles.calc__range}
                            onMouseDown={props.onMouseDown}
                        ...
                        renderThumb={({ props, isDragged }) => (
                          ...
                          >
                            <span>{firstPayment[props.key]}</span>
                          </div>
                        )}
                      />
                      ...
                      <ul>
                        <li 
                          className={term === 104 ? styles.active : ''}
                          onClick={() => setTerm(104)}
                        >
                          24 мес.
                          </li>
                        <li
                          className={term === 52 ? styles.active : ''}
                          onClick={() => setTerm(52)}
                        >
                          12 мес.
                          </li>
                  ...
                  <div className={styles.calc__right}>
                    <ul className={styles.calc__right_top}>
                      <li>
                        <span>
                          {Math.round((((autoPrice * 1.8) + ((autoPrice * 1.8) * 0.08)) - firstPayment) / term).toLocaleString()} $ /
                           {Math.round(Math.round((((autoPrice * 1.8) + ((autoPrice * 1.8) * 0.08)) - firstPayment) / term) * usd).toLocaleString()} грн
                        </span>
                        сумма еженедельного платежа включает стоимость ОСАГО и КАСКО + мы компенсируем первое ТО
                      </li>
                    </ul>

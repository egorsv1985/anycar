import styles from '../styles/Header.module.scss'

import {useEffect, useState} from "react";

//Components
import Button from "./Button";
import Modal from "./Modal";
import Menu from './Menu';

//Api
import {useDispatch, useSelector} from "react-redux";
import {fetchMenu} from "../redux/actions";

export default function Header({white, lang, translateLink}) {
  const [scrolled, setScrolled] = useState(false);
  const [open, setOpen] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const dispatch = useDispatch()
  const menu = useSelector((state) => state.menu)

  const handleScroll = () => {
    if (window.pageYOffset > 200) {
      setScrolled(true)
    } else {
      setScrolled(false)
    }
  }

  useEffect(() => {
    dispatch(fetchMenu(lang));
    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [])

  return (
    <>
      {menu.length > 0 && 
        <>
          <header className={`${white ? styles.white: ''} ${styles.header}`}>
            <div className="container">
    
            <button
              className={`hamburger hamburger--spin ${isMenuOpen ? 'is-active' : ''} ${styles.burger}`}
              type="button"
              onClick={() => setIsMenuOpen(!isMenuOpen)}
            >
              <span className="hamburger-box">
                <span className="hamburger-inner" />
              </span>
            </button>
              
              <a href={lang === 'ru' ? '/' : '/uk/'} className={styles.header__logo}>
                <img src={white ? "/images/logo-white.png" : "/images/logo.png"} alt="logo" />
              </a>
    
              <a href={lang === 'ru' ? '/' : '/uk/'} className={styles.header__logo_black}>
                <img src="/images/logo.png" alt="logo" />
              </a>
    
              <nav className={styles.header__nav}>
                <ul>
                  <li>
                    {menu[0].title}
                    <ul className={styles.dropdown}>
                      <li>
                        <a href={menu[1].url}>{menu[1].title}</a>
                      </li>
                      {/* <li>
                        <a href={menu[2].url}>{menu[2].title}</a>
                      </li> */}
                      <li>
                        <a href={menu[3].url}>{menu[3].title}</a>
                      </li>
                      <li>
                        <a href={menu[4].url}>{menu[4].title}</a>
                      </li>
                    </ul>
                  </li>
    
                  <li>
                    {menu[5].title}
                    <ul className={styles.dropdown}>
                      <li>
                        <a href={menu[6].url}>{menu[6].title}</a>
                      </li>
                      <li>
                        <a href={menu[7].url}>{menu[7].title}</a>
                      </li>
                    </ul>
                  </li>

                  <li>
                    {menu[8].title}
                    <ul className={styles.dropdown}>
                      <li>
                        <a href={menu[9].url}>{menu[9].title}</a>
                      </li>
                      <li>
                        <a href={menu[10].url}>{menu[10].title}</a>
                      </li>
                    </ul>
                  </li>
    
                  <li>
                    <a href={menu[11].url}>{menu[11].title}</a>
                  </li>
                </ul>
              </nav>
    
              <div className={styles.header__right}>
                <a href={`tel: ${menu[12].title}`} className={styles.header__phone}>{menu[12].title}</a>
                <Button onClick={() => setOpen(true)}  text={lang === 'ru' ? "Консультация" : "Консультація"} white={white} />
                <a href={translateLink} className={styles.header__lang}>{lang === 'ru' ? "Uk" : "Ru"}</a>
              </div>
    
              <a className={styles.phone} href={`tel: ${menu[12].title}`}>
                <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M21.9122 27.1245C20.7685 27.1245 19.1618 26.7108 16.756 25.3667C13.8304 23.726 11.5675 22.2114 8.65772 19.3092C5.85225 16.5055 4.48702 14.6903 2.57628 11.2133C0.417684 7.28754 0.785652 5.22973 1.19698 4.35024C1.68682 3.29907 2.40987 2.67036 3.34444 2.04633C3.87527 1.69854 4.43702 1.4004 5.02257 1.15571C5.08116 1.13051 5.13565 1.10649 5.18429 1.08481C5.47433 0.954144 5.91378 0.756683 6.47042 0.96762C6.8419 1.10707 7.17354 1.39243 7.69269 1.90512C8.75733 2.95512 10.2122 5.2936 10.7489 6.44204C11.1093 7.21606 11.3478 7.727 11.3483 8.30004C11.3483 8.97094 11.0108 9.48833 10.6013 10.0467C10.5245 10.1516 10.4483 10.2518 10.3745 10.3491C9.92862 10.935 9.83077 11.1043 9.89522 11.4067C10.0259 12.0143 11.0003 13.8231 12.6017 15.4209C14.203 17.0188 15.9597 17.9317 16.5696 18.0618C16.8849 18.1291 17.0577 18.0272 17.6624 17.5655C17.7491 17.4993 17.8382 17.4307 17.9314 17.3622C18.556 16.8975 19.0493 16.5688 19.7044 16.5688H19.7079C20.278 16.5688 20.7661 16.8161 21.5747 17.2239C22.6294 17.7559 25.0382 19.192 26.0946 20.2579C26.6085 20.7758 26.895 21.1063 27.0351 21.4772C27.246 22.0356 27.0474 22.4733 26.9179 22.7663C26.8962 22.8149 26.8722 22.8682 26.847 22.9274C26.6003 23.5119 26.3005 24.0725 25.9511 24.602C25.3282 25.5336 24.6972 26.2549 23.6437 26.7454C23.1027 27.0012 22.5106 27.1309 21.9122 27.1245Z" fill="#0D0A46"/>
                </svg>
              </a>
            </div>
          </header>
          <header className={`${styles.header} ${styles.scrolled} ${scrolled ? styles.active : ''}`}>
            <div className="container">
            <button
              className={`hamburger hamburger--spin ${isMenuOpen ? 'is-active' : ''} ${styles.burger}`}
              type="button"
              onClick={() => setIsMenuOpen(!isMenuOpen)}
            >
              <span className="hamburger-box">
                <span className="hamburger-inner" />
              </span>
            </button>
            
              <a href={lang === 'ru' ? '/' : '/uk/'} className={styles.header__logo}>
                <img src="/images/logo.png" alt="logo" />
              </a>
    
              <nav className={styles.header__nav}>
                <ul>
                  <li>
                    {menu[0].title}
                    <ul className={styles.dropdown}>
                      <li>
                        <a href={menu[1].url}>{menu[1].title}</a>
                      </li>
                      {/* <li>
                        <a href={menu[2].url}>{menu[2].title}</a>
                      </li> */}
                      <li>
                        <a href={menu[3].url}>{menu[3].title}</a>
                      </li>
                      <li>
                        <a href={menu[4].url}>{menu[4].title}</a>
                      </li>
                    </ul>
                  </li>

                  <li>
                    {menu[5].title}
                    <ul className={styles.dropdown}>
                      <li>
                        <a href={menu[6].url}>{menu[6].title}</a>
                      </li>
                      <li>
                        <a href={menu[7].url}>{menu[7].title}</a>
                      </li>
                    </ul>
                  </li>

                  <li>
                    {menu[8].title}
                    <ul className={styles.dropdown}>
                      <li>
                        <a href={menu[9].url}>{menu[9].title}</a>
                      </li>
                      <li>
                        <a href={menu[10].url}>{menu[10].title}</a>
                      </li>
                    </ul>
                  </li>

                  <li>
                    <a href={menu[11].url}>{menu[11].title}</a>
                  </li>
                </ul>
              </nav>
    
              <div className={styles.header__right}>
                <a href={`tel: ${menu[12].title}`} className={styles.header__phone}>{menu[12].title}</a>
                <Button onClick={() => setOpen(true)} text={lang === 'ru' ? "Консультация" : "Консультація"} />
                <a href={translateLink} className={styles.header__lang}>{lang === 'ru' ? "Uk" : "Ru"}</a>
              </div>
    
              <a className={styles.phone} href={`tel: ${menu[12].title}`}>
                <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M21.9122 27.1245C20.7685 27.1245 19.1618 26.7108 16.756 25.3667C13.8304 23.726 11.5675 22.2114 8.65772 19.3092C5.85225 16.5055 4.48702 14.6903 2.57628 11.2133C0.417684 7.28754 0.785652 5.22973 1.19698 4.35024C1.68682 3.29907 2.40987 2.67036 3.34444 2.04633C3.87527 1.69854 4.43702 1.4004 5.02257 1.15571C5.08116 1.13051 5.13565 1.10649 5.18429 1.08481C5.47433 0.954144 5.91378 0.756683 6.47042 0.96762C6.8419 1.10707 7.17354 1.39243 7.69269 1.90512C8.75733 2.95512 10.2122 5.2936 10.7489 6.44204C11.1093 7.21606 11.3478 7.727 11.3483 8.30004C11.3483 8.97094 11.0108 9.48833 10.6013 10.0467C10.5245 10.1516 10.4483 10.2518 10.3745 10.3491C9.92862 10.935 9.83077 11.1043 9.89522 11.4067C10.0259 12.0143 11.0003 13.8231 12.6017 15.4209C14.203 17.0188 15.9597 17.9317 16.5696 18.0618C16.8849 18.1291 17.0577 18.0272 17.6624 17.5655C17.7491 17.4993 17.8382 17.4307 17.9314 17.3622C18.556 16.8975 19.0493 16.5688 19.7044 16.5688H19.7079C20.278 16.5688 20.7661 16.8161 21.5747 17.2239C22.6294 17.7559 25.0382 19.192 26.0946 20.2579C26.6085 20.7758 26.895 21.1063 27.0351 21.4772C27.246 22.0356 27.0474 22.4733 26.9179 22.7663C26.8962 22.8149 26.8722 22.8682 26.847 22.9274C26.6003 23.5119 26.3005 24.0725 25.9511 24.602C25.3282 25.5336 24.6972 26.2549 23.6437 26.7454C23.1027 27.0012 22.5106 27.1309 21.9122 27.1245Z" fill="#0D0A46"/>
                </svg>
              </a>
            </div>
          </header>
    
          <Modal open={open} setOpen={setOpen} title={lang === 'ru' ? "Оставьте заявку" : "Залиште заявку"} name={true} phone={true} lang={lang} />
          <Menu isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} menu={menu} lang={lang} translateLink={translateLink} />
        </>
      }
    </>
  )
}
